...
saveRDS(ols.model, paste0(model.dump.string, "ols_model.RDS"))
...
saveRDS(freq.ridge.model, paste0(model.dump.string, "freq_ridge_model.RDS"))
...
freq.lasso.model <- glmnet(X, y, alpha = 1, lambda = opt.lasso.lambda, intercept = FALSE)

saveRDS(freq.lasso.model, paste0(model.dump.string, "freq_lasso_model.RDS"))
...
saveRDS(blm.sims, paste0(model.dump.string, "blm_sims.RDS"))
...
saveRDS(ridge.sims, paste0(model.dump.string, "ridge_sims.RDS"))
...
saveRDS(lasso.sims, paste0(model.dump.string, "lasso_sims.RDS"))
