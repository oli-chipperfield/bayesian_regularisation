...
source("evaluate_cross_validation_basic.R")
...
compare_prior_rmspe <- function(model) {
  ...
  prior.mods <- prior.mods[grepl("basic", prior.mods) == TRUE]
...
prior.tests.basic <- list(ridge.prior.pp.rmspe = ridge.prior.pp.rmspe,
...
saveRDS(prior.tests.basic, "prior_test_results_basic.RDS")
