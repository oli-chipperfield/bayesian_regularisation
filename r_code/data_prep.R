...
#### Add polybnomials to certain terms
...
d[, indus_sq := indus^2]
d[, nox_sq := nox^2]
d[, ptratio_sq := ptratio^2]
d[, lstat_sq := lstat^2]
d[, rm_sq := rm^2]

#plot(d$ptratio, d$ptratio_sq)#, xlim = c(0, 3.1), ylim = c(0, 9.5))

d.long <- data.table(gather(d, var, value, names(d)[names(d) != "medv"]))
# # # 
#ggplot(d.long, aes(x = value, y = log(medv))) + geom_point() + facet_wrap(~ var, scales = "free")
# # # 
